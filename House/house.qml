...
Page 
{
    ...
    Component 
    {
        ...
        MapQuickItem 
        {
            ...
            sourceItem: Image 
            {
                ...
                source: 
                {
                    var path = backend.GetPluginDir() + "/house/"
                    if (status == 0)
                        return path + "ampel_green.png"
                    else if(status == 1)
                        return path + "ampel_yellow.png"
                    else 
                        return path + "ampel_red.png" 
                }
            }
        }
    }
    ...
    function loadLocations()
    {
        ...
        { 
            "tags" : "#house", 
            "latitude" : src.position.coordinate.latitude == src.position.coordinate.latitude ? src.position.coordinate.latitude : 0,
            "longitude" : src.position.coordinate.longitude == src.position.coordinate.longitude ? src.position.coordinate.longitude : 0,
            "distance" : 10000000
        }));
    }
